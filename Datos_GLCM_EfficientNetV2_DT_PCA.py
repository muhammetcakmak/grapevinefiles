...
def main():
    ...
    for fold, (train_indices, test_indices) in enumerate(stratified_kfold.split(train_features, train_labels)):
        ...
        fold_probabilities = dt_classifier.predict_proba(fold_test_features)  # Decision Tree'de proba çıktısı yok, ama işlem uyumluluğu için burada bırakıldı
        ...
        # Decision Tree'de probability hesaplanamadığı için bu kısımlar görselleştirme için kullanılmamalı
    ...
    # Precision-Recall Eğrisi, Decision Tree'de kullanılmaz
    # plt.figure(figsize=(8, 8))

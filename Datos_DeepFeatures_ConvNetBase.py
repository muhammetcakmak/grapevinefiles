...
def preprocess_image(image_path):
    ...
    image_resized = cv2.resize(image, (224, 224))  # Resize image to the required size for ResNet50
...
def main(image_path):
    ...
    layer_outputs_values = activation_model.predict(preprocessed_image)
    layer_names = [layer.name for layer in base_model.layers]
    visualize_layer_outputs(layer_outputs_values[:12], layer_names[:12])  # Visualize first 12 layers for brevity

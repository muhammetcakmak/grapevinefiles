...
# Load the InceptionV3 model and create a feature extractor model
base_model = InceptionV3(weights='imagenet', include_top=False, pooling='avg')
model = Model(inputs=base_model.input, outputs=base_model.output)

def extract_features(image):
    # Extract features using InceptionV3
    image_resized = cv2.resize(image, (299, 299))  # Appropriate size for InceptionV3
    ...
    # Combine InceptionV3 and color space features
...
def load_data(data_path):
    ...
    for class_folder in os.listdir(data_path):
        ...
        for img_name in os.listdir(class_path):
            ...
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # OpenCV uses BGR, so convert to RGB
...
def main():
    ...
    selector = SelectKBest(score_func=f_classif, k='all')  # Set k to a suitable value
    ...
    for fold, (train_indices, test_indices) in enumerate(stratified_kfold.split(train_features, train_labels)):
        ...
        # LightGBM model
        lgb_classifier = lgb.LGBMClassifier(objective='multiclass', num_class=len(np.unique(train_labels)), random_state=42)

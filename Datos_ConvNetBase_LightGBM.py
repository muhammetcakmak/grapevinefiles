...
convnext_base_model = ConvNeXtBase(weights='imagenet', include_top=False, pooling='avg')
convnext_model = Model(inputs=convnext_base_model.input, outputs=convnext_base_model.output)

def extract_features(image):
    # ConvNeXtBase ile özellik çıkarımı
    image_resized_convnext = cv2.resize(image, (224, 224))  # ConvNeXtBase için uygun boyut
    image_preprocessed_convnext = preprocess_input_convnext(image_resized_convnext)
    image_preprocessed_convnext = np.expand_dims(image_preprocessed_convnext, axis=0)
    convnext_features = convnext_model.predict(image_preprocessed_convnext).flatten()
    
    return convnext_features
